...
#    an authentication (username/password) frame
#    a multi-factor authentication (information factor) frame
#    the restircted applicaiton frame
# Users must pass all authentication steps to access the restricted app
...
class MultiFactorAuth(tk.Tk):
  ...
  def set_authentication(self, user="admin", pw="secret"):
    self.username = user
    self.password = pw

  # save security question/answer that the user wants to use
  def set_multiFactorAuthentication(self, user_question="unknwon", user_answer="unknown"):
    self.security_question = user_question
    self.answer = user_answer

  # create an instance of this class with an authentication window
  def __init__(self):
    tk.Tk.__init__(self)
    ...
    # create authentication frame and place widgets in it
    ...
    self.btn_login = tk.Button(self.frame_login, text="LOG IN", command=self.authenticate)
    self.btn_login.pack(padx=175, pady=20)
    
    # show this authentication frame
    self.frame_login.tkraise()

  # get the authentication information and if authenticated, 
  # create and show the multi-factor authentication frame
  def authenticate(self):
    ...
    if (student_username == self.username and  student_password == self.password):
        # authenticated - create multi-factor authentication frame and widgets
        self.frame_auth = tk.Frame(self, bg="plum4")
        self.title("Multi-Factor Authenticate")
        ...
        self.btn_auth = tk.Button(self.frame_auth, text="AUTHENTICATE", command=self.multiFactorAuthenticate)
  ...
  # get the multi-factor authentication information and if authenticated,
  # create and show the restricted app
  def multiFactorAuthenticate(self):
 ...
 # Reset 
  def reset_authentication(self):
    ...
    if (student_username == self.username and  student_password == self.password):
        ...
        self.btn_reset_auth = tk.Button(self.frame_reset, text='SET new Username and Password', command=self.set_authentication_UI)
